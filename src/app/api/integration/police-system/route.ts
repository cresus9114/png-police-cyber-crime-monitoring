  ...
  try {
    ...
    const transformedCases = cases.map((case_) => ({
      ...
      suspects: case_.suspects.map(suspect => ({
        ...
        knownLocation: suspect.location,
      })),
...
async function handleCaseLinked(data: any) {
  ...
  await db.cyberCase.update({
    where: { caseId: cyberCaseId },
    data: {
      parentCaseId: mainSystemCaseId,
    },
  });
  ...
  await db.caseUpdate.create({
    data: {
      ...
      content: `This cyber crime case has been linked to main system case ${mainSystemCaseId} (${linkType})`,
  ...
  if (case_?.assignedTo) {
    await db.notification.create({
      data: {
        ...
        type: "CASE_UPDATE",
        title: "Case Linked to Main System",
        message: `Your cyber crime case ${cyberCaseId} has been linked to main system case ${mainSystemCaseId}`,
      },
    });
  }
}
...
async function handleOfficerTransferred(data: any) {
  ...
  if (fromDepartment === "Cyber Crime Unit" && toDepartment !== "Cyber Crime Unit") {
  ...
  await db.auditLog.create({
    data: {
      ...
      action: "TRANSFER",
      resource: "user",
      oldValues: { department: fromDepartment },
      newValues: { department: toDepartment, transferDate },
    },
  });
}

// Handle case priority changes from main system
async function handleCasePriorityChanged(data: any) {
  ...
  await db.caseUpdate.create({
    data: {
      ...
      content: `Case priority changed to ${newPriority} by ${authorizedBy}. Reason: ${reason}`,
  ...
  if (updatedCase.assignedTo) {
    await db.notification.create({
      data: {
        ...
        message: `Case ${caseId} priority has been changed to ${newPriority}`,
...
async function handleUnitRestructure(data: any) {
